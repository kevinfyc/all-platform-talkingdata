...
public class StringFast
{
    ...
    public StringFast Append(float value)
    {
        ...
        int nbFloatDigits = 0;
        while (System.Math.Abs(value - System.Math.Round(value)) > 0.00001f)
        {
            value *= 10;
            nbFloatDigits++;
        }
        var valueRound = System.Math.Round(value);

        // Copy the digits in reverse order
        int nbChars = 0;
        do
        {
            m_chars[m_charsCount++] = (char)('0' + valueRound % 10);
            valueRound /= 10;
            nbChars++;
        ...
        } while (valueRound != 0 || nbChars <= nbFloatDigits + 1);
